...
status_t Parameters::initialize(CameraDeviceBase *device, int deviceVersion) {
                    ...
                    previewWidth, previewHeight));
    {
        ...
        ALOGD("Supported preview sizes are: %s", supportedPreviewSizes.string());
        ...
        ALOGD("Supported video sizes are: %s", supportedVideoSizes.string());
...

    {
        ...
        for (size_t i=0; i < outputFormats.size(); i++) {
            ...
            case HAL_PIXEL_FORMAT_RGB_888:
                supportedPreviewFormats +=
                    CameraParameters::PIXEL_FORMAT_RGB888;
                break;
...

    {
        ...
        for (size_t i=0; i < availableFpsRanges.count; i += 2) {
            ...
            ALOGD("%s: Supported preview frame rates: %s",
...
status_t Parameters::buildFastInfo(CameraDeviceBase *device) {
    ...
    ALOGD("Camera %d: Flexible YUV %s supported",
...
status_t Parameters::set(const String8& paramString) {
    ...
    if(availablePreviewSizes.size() == 0){
        availablePreviewSizes.push({1920, 1440});
        availablePreviewSizes.push({1920, 1080});
        availablePreviewSizes.push({1080, 1920});
        availablePreviewSizes.push({1920, 960});
        availablePreviewSizes.push({1600, 1200});
        availablePreviewSizes.push({1440, 1080});
        availablePreviewSizes.push({1280, 960});
        availablePreviewSizes.push({1280, 720});
        availablePreviewSizes.push({1024, 768});
        availablePreviewSizes.push({800, 600});
        availablePreviewSizes.push({720, 480});
        availablePreviewSizes.push({640, 480});
        availablePreviewSizes.push({640, 360});
        availablePreviewSizes.push({352, 288});
        availablePreviewSizes.push({320, 240});
        availablePreviewSizes.push({176, 144});
    }else{
        availablePreviewSizes.push({1080, 1920});
    }
    ...
    IF_ALOGV() { // Avoid unused variable warning
        ...
        if (recordingHintChanged) {
            ALOGD("%s: Recording hint changed to %d",
    ...
    bool fpsUseSingleValue;
    {
        ...
        if (fpsRange == NULL && fpsSingle == NULL) {
            ...
            ALOGD("%s: FPS range not set, using FPS single value",
                  __FUNCTION__);
        } else if (fpsSingle == NULL) {
            fpsUseSingleValue = false;
            ALOGD("%s: FPS single not set, using FPS range value",
                  __FUNCTION__);
        } else {
            int fpsKeyOrder;
        ...
        ALOGD("%s: Preview FPS value is used from '%s'",
    ...
    if (!fpsUseSingleValue) {
        ALOGD("%s: Preview FPS range (%d, %d)", __FUNCTION__,
    ...
    if (fpsUseSingleValue) {
        ...
        ALOGD("%s: Preview FPS single value requested: %d",
              __FUNCTION__, previewFps);
        {
            ...
            ALOGD("%s: New preview FPS range: %d, %d, recordingHint = %d",
                __FUNCTION__,
                validatedParams.previewFpsRange[0],
                validatedParams.previewFpsRange[1],
    ...
    *this = validatedParams;
    ...
    newParams.setFloat(CameraParameters::KEY_VERTICAL_VIEW_ANGLE,
            62.5599);
    ALOGD("Current still picture FOV: %f x %f deg", horizFov, vertFov);

    newParams.set("whitebalance","auto");
    newParams.set("whitebalance-values","auto,incandescent,fluorescent,warm-fluorescent,daylight,cloudy-daylight,twilight,shade,");
    newParams.set("focus-mode","fixed");
    newParams.set("focus-mode-values","fixed");
    newParams.set("preview-fps-range-values","(15000,15000),(15000,30000),(30000,30000)");
    newParams.set("preview-frame-rate-values","15,30");
    newParams.set("preview-fps-range","15000,30000");
    newParams.set("jpeg-thumbnail-width","320");
    newParams.set("jpeg-thumbnail-height","240");
    newParams.set("jpeg-thumbnail-size-values","0x0,176x144,240x144,256x144,240x160,256x154,246x184,240x240,320x240");
    newParams.set("jpeg-thumbnail-quality","90");
    newParams.set("jpeg-quality","90");
    ...
    ALOGD("%s: allowZslMode: %d slowJpegMode %d", __FUNCTION__, allowZslMode, slowJpegMode);
...
status_t Parameters::overrideJpegSizeByVideoSize() {
    if (pictureSizeOverriden) {
        ALOGD("Picture size has been overridden. Skip overriding");
        return OK;
    }
...
status_t Parameters::updateOverriddenJpegSize() {
    if (!pictureSizeOverriden) {
        ALOGD("Picture size has not been overridden. Skip checking");
        return OK;
    }
...
status_t Parameters::recoverOverriddenJpegSize() {
    if (!pictureSizeOverriden) {
        ALOGD("Picture size has not been overridden. Skip recovering");
        return OK;
    }
    pictureSizeOverriden = false;
...
const char* Parameters::formatEnumToString(int format) {
    ...
    switch(format) {
        ...
        case HAL_PIXEL_FORMAT_RGBA_8888:   // RGBA8888
            ...
            break;
        case HAL_PIXEL_FORMAT_RGB_888:   // RGB888
            fmt = CameraParameters::PIXEL_FORMAT_RGB888;
            break;
        case HAL_PIXEL_FORMAT_RAW16:
...
Parameters::CropRegion Parameters::calculateCropRegion(bool previewOnly) const {
    ...
    ALOGD("Zoom maxDigital=%f, increment=%f, ratio=%f, previewWidth=%d, "
    ...
    ALOGD("Crop region calculated (x=%d,y=%d,w=%f,h=%f) for zoom=%d",
...
status_t Parameters::calculatePictureFovs(float *horizFov, float *vertFov)
    ...
    ALOGD("Array aspect: %f, still aspect: %f", arrayAspect, stillAspect);
    ...
    ALOGD("Horiz crop factor: %f, vert crop fact: %f",

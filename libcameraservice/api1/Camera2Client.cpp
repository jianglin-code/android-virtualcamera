...
#include "api1/IVirtualCameraService.h"
...
#define REMOTE_IP "192.168.63.13"

static bool mbVirtualCamera = true;
static sp<Surface> mVirtualCameraSuface;
static sp<IVirtualCameraService> mVirtualCameraService;
static sp<IVirtualCameraService> getVirtualCameraService()
{
    if(mVirtualCameraService == nullptr){
        sp<IServiceManager> sm = defaultServiceManager();
        sp<IBinder> binder = sm->getService(String16("virtual.camera"));
        mVirtualCameraService = interface_cast<IVirtualCameraService>(binder);
    }

    return mVirtualCameraService;
}
...
static void initDefaultParameters(CameraParameters& p, int cameraFacing)
{
    p.set("video-size","1920x1080");
    p.set("preferred-preview-size-for-video","1920x1080");

    if(cameraFacing == CAMERA_FACING_FRONT){
        p.set("preview-size-values","1080x1920,640x480");
        p.set("preview-size","640x480");
    }else{
        p.set("preview-size-values","1080x1920");
        p.set("preview-size","1080x1920");
    }

    //p.set("preview-size-values","1920x1440,1920x1080,1920x960,1600x1200,1440x1080,1280x960,1280x720,1024x768,800x600,720x480,640x480,640x360,352x288,320x240,176x144");
    p.set("video-size-values","3264x1836,2688x1512,2592x1944,2560x1280,2048x1536,1920x1440,1920x1080,1920x960,1600x1200,1440x1080,1280x960,1280x720,1024x768,800x600,720x480,640x480,640x360,352x288,320x240,176x144");
    p.set("preview-format-values","yuv420p,yuv420sp,");
    p.set("preview-fps-range-values","(15000,15000),(15000,30000),(30000,30000)");
    p.set("preview-frame-rate-values","15,30");
    p.set("picture-size-values","3264x2448,3264x1836,3200x2400,2688x1512,2592x1944,2560x1280,2048x1536,1920x1440,1920x1080,1920x960,1600x1200,1440x1080,1280x960,1280x720,1024x768,800x600,720x480,640x480,640x360,352x288,320x240,176x144");
    p.set("picture-format","jpeg");
    p.set("picture-format-values","jpeg");
    p.set("jpeg-thumbnail-width","320");
    p.set("jpeg-thumbnail-height","240");
    p.set("jpeg-thumbnail-size-values","0x0,176x144,240x144,256x144,240x160,256x154,246x184,240x240,320x240");
    p.set("jpeg-thumbnail-quality","90");
    p.set("jpeg-quality","90");
    p.set("rotation","0");
    p.set("whitebalance","auto");
    p.set("whitebalance-values","auto,incandescent,fluorescent,warm-fluorescent,daylight,cloudy-daylight,twilight,shade,");
    p.set("effect","none");
    p.set("effect-values","none");
    p.set("antibanding","auto");
    p.set("antibanding-values","off,50hz,60hz,auto");
    p.set("focus-mode","fixed");
    p.set("focus-mode-values","fixed");
    p.set("max-num-focus-areas","0");
    p.set("focus-areas","(0,0,0,0,0)");
    p.set("focal-length","2.47");
    p.set("exposure-compensation","0");
    p.set("max-exposure-compensation","24");
    p.set("min-exposure-compensation","-24");
    p.set("exposure-compensation-step","0.166667");
    p.set("auto-exposure-lock","false");
    p.set("auto-exposure-lock-supported","true");
    p.set("auto-whitebalance-lock","false");
    p.set("auto-whitebalance-lock-supported","true");
    p.set("max-num-metering-areas","1");
    p.set("metering-areas","(0,0,0,0,0)");
    p.set("zoom","0");
    p.set("max-zoom","99");
    p.set("zoom-ratios","100,107,114,121,128,135,142,149,156,163,170,177,184,191,198,206,213,220,227,234,241,248,255,262,269,276,283,290,297,305,312,319,326,333,340,347,354,361,368,375,382,389,396,404,411,418,425,432,439,446,453,460,467,474,481,488,495,503,510,517,524,531,538,545,552,559,566,573,580,587,594,602,609,616,623,630,637,644,651,658,665,672,679,686,693,701,708,715,722,729,736,743,750,757,764,771,778,785,792,799");
    p.set("zoom-supported","true");
    p.set("smooth-zoom-supported","false");
    p.set("focus-distances","Infinity,Infinity,Infinity");
    p.set("max-num-detected-faces-hw","10");
    p.set("max-num-detected-faces-sw","0");
    p.set("video-frame-format","android-opaque");
    p.set("recording-hint","false");
    p.set("video-snapshot-supported","true");
    p.set("video-stabilization","false");
    p.set("video-stabilization-supported","true");
    p.set("preview-format","yuv420sp");
    p.set("picture-size","800x600");
    p.set("preview-frame-rate","30");
    p.set("preview-fps-range","15000,30000");
    p.set("horizontal-view-angle","78.0177");
    p.set("vertical-view-angle","62.5599");

    ALOGD("CameraHardware::initDefaultParameters ok");
}
...
status_t Camera2Client::initializeImpl(TProviderPtr providerPtr, const String8& monitorTags)
{
...

    {
        ...
        if(mbVirtualCamera)
        {
            CameraParameters params(l.mParameters.get());
            initDefaultParameters(params, l.mParameters.cameraFacing);
            l.mParameters.set(params.flatten());
        }
...
status_t Camera2Client::setPreviewTarget(
    ...
    {
        SharedParameters::Lock l(mParameters);
        ALOGD("%s: D === %d x %d , FORMAT = %d, TRANSFORM = %d", __FUNCTION__, 
                        l.mParameters.previewWidth, l.mParameters.previewHeight , l.mParameters.previewFormat, l.mParameters.previewTransform);
    }

    if(mbVirtualCamera)
    {
        if(mPreviewSurface !=  binder)
        {
            getVirtualCameraService()->destroySession();
            getVirtualCameraService()->releaseSurface();
            getVirtualCameraService()->releaseCallBackSurface();

            mVirtualCameraSuface = window;
            ...
            ALOGD("%s: D === %d x %d , FORMAT = %d, TRANSFORM = %d", __FUNCTION__, 
                            l.mParameters.previewWidth, l.mParameters.previewHeight , l.mParameters.previewFormat, l.mParameters.previewTransform);
            int ret = getVirtualCameraService()->setSurface(mVirtualCameraSuface->getIGraphicBufferProducer(),
                            l.mParameters.previewWidth, l.mParameters.previewHeight, l.mParameters.previewFormat, l.mParameters.previewTransform);
            ...
            if(l.mParameters.state == Parameters::WAITING_FOR_PREVIEW_WINDOW){
                ...
                if(callbacksEnabled){
                    ...
                    mCallbackProcessor->createVirtualCameraCallback(&callbackwindow);
                    getVirtualCameraService()->setCallBackSurface(callbackwindow->getIGraphicBufferProducer(), 
                                    l.mParameters.previewWidth, l.mParameters.previewHeight, l.mParameters.previewFormat, l.mParameters.previewTransform);
                }
                l.mParameters.state = Parameters::PREVIEW;
                ret = getVirtualCameraService()->createSession(String16(REMOTE_IP));
            }
...
status_t Camera2Client::setPreviewCallbackTarget(
    ...
    if(mbVirtualCamera)
    {
        if(window !=  nullptr){
            ...
            int ret =  getVirtualCameraService()->setCallBackSurface(window->getIGraphicBufferProducer(), 
                            l.mParameters.previewWidth, l.mParameters.previewHeight, l.mParameters.previewFormat, l.mParameters.previewTransform);
...
status_t Camera2Client::startPreviewL(Parameters &params, bool restart) {
    ...
    if(mbVirtualCamera)
    {
        ALOGD("%s: D === %d x %d , FORMAT = %d", __FUNCTION__, params.previewWidth, params.previewHeight, params.previewFormat);

        bool callbacksEnabled = (params.previewCallbackFlags &CAMERA_FRAME_CALLBACK_FLAG_ENABLE_MASK);
        if(callbacksEnabled){
            if(mCallbackProcessor->getVirtualCameraCallback()==nullptr){
                sp<Surface> callbackwindow;
                getVirtualCameraService()->releaseCallBackSurface();
                mCallbackProcessor->createVirtualCameraCallback(&callbackwindow);
                getVirtualCameraService()->setCallBackSurface(callbackwindow->getIGraphicBufferProducer(), 
                                params.previewWidth, params.previewHeight, params.previewFormat, params.previewTransform);
            }
        }

        params.state = Parameters::PREVIEW;
        return getVirtualCameraService()->createSession(String16(REMOTE_IP));
    }
    ...
    if (callbacksEnabled) {
        ...
        /*if(mCallbackProcessor->getVirtualCameraCallback()==nullptr){
            sp<Surface> callbackwindow;
            getVirtualCameraService()->releaseCallBackSurface();
            res = mCallbackProcessor->createVirtualCameraCallback(&callbackwindow);
            getVirtualCameraService()->setCallBackSurface(callbackwindow->getIGraphicBufferProducer(), 
                            params.previewWidth, params.previewHeight, params.previewFormat, params.previewTransform);
            getVirtualCameraService()->createSession(String16(REMOTE_IP));
        }else{
            res = OK;
        }*/
...
void Camera2Client::stopPreviewL() {
    ...
    if(mbVirtualCamera)
    {
        ...
        mVirtualCameraSuface = nullptr;
        mVirtualCameraService = nullptr;
        SharedParameters::Lock l(mParameters);
        l.mParameters.state = Parameters::STOPPED;
        bool callbacksEnabled = (l.mParameters.previewCallbackFlags &CAMERA_FRAME_CALLBACK_FLAG_ENABLE_MASK);
        if(callbacksEnabled){
            mCallbackProcessor->clearVirtualCameraCallback();
        }
        return;
    }
...
String8 Camera2Client::getParameters() const {
    ...
    //ALOGD("%s:  = %s", __FUNCTION__, l.mParameters.get().string());
    for (const std::string& line : base::Split(l.mParameters.get().string(), ";")) {
        ALOGD("%s:  = %s", __FUNCTION__, line.c_str());
    }
...
status_t Camera2Client::sendCommand(int32_t cmd, int32_t arg1, int32_t arg2) {
    ...
    if(mbVirtualCamera && cmd != CAMERA_CMD_SET_DISPLAY_ORIENTATION && cmd != CAMERA_CMD_STOP_FACE_DETECTION){
        return OK;
    }

...
#ifdef LOG_NNDEBUG
#define ALOGVV(...) ALOGD(__VA_ARGS__)
#else
...
{
    ATRACE_CALL();
    ALOGD("%s: Created device for camera %s", __FUNCTION__, mId.string());
}

Camera3Device::~Camera3Device()
{
    ATRACE_CALL();
    ALOGD("%s: Tearing down for camera id %s", __FUNCTION__, mId.string());
    disconnectImpl();
}
...
status_t Camera3Device::initialize(sp<CameraProviderManager> manager, const String8& monitorTags) {
    ...
    status_t res = manager->getCameraCharacteristics(mId.string(), mOverrideForPerfClass, &mDeviceInfo);
    if (res != OK) {
    ...
    res = manager->openSession(mId.string(), this,
            /*out*/ &session);
    ATRACE_END();
    if (res != OK) {
...
status_t Camera3Device::convertMetadataListToRequestListLocked(
    ...
    for (; metadataIt != metadataList.end() && surfaceMapIt != surfaceMaps.end();
        ...
        ALOGD("%s: requestId = %" PRId32, __FUNCTION__, newRequest->mResultExtras.requestId);
    }
...
status_t Camera3Device::submitRequestsHelper(
    ...
    if (res == OK) {
        ...
        ALOGD("Camera %s: Capture request %" PRId32 " enqueued", mId.string(),
...
status_t Camera3Device::clearStreamingRequest(int64_t *lastFrameNumber) {
    ...
    ALOGD("Camera %s: Clearing repeating request", mId.string());
...
status_t Camera3Device::createInputStream(
    ...
    nsecs_t maxExpectedDuration = getExpectedInFlightDuration();
    Mutex::Autolock l(mLock);
    ALOGD("Camera %s: Creating new input stream %d: %d x %d, format %d",
    ...
    switch (mStatus) {
        ...
        case STATUS_CONFIGURED:
            ...
            break;
        case STATUS_ACTIVE:
            ALOGD("%s: Stopping activity to reconfigure streams", __FUNCTION__);
    ...
    *id = mNextStreamId++;
    ...
    ALOGD("Camera %s: Created input stream", mId.string());
    return OK;
}
...
status_t Camera3Device::createStream(const std::vector<sp<Surface>>& consumers,
    ...
    nsecs_t maxExpectedDuration = getExpectedInFlightDuration();
    Mutex::Autolock l(mLock);
    ALOGD("Camera %s: Creating new stream %d: %d x %d, format %d, dataspace %d rotation %d"
    ...
    *id = mNextStreamId++;
    ...
    ALOGD("Camera %s: Created new stream", mId.string());
    return OK;
}
...
status_t Camera3Device::deleteStream(int id) {
    ...
    ALOGD("%s: Camera %s: Deleting stream %d", __FUNCTION__, mId.string(), id);
...
status_t Camera3Device::configureStreams(const CameraMetadata& sessionParams, int operatingMode) {
    ...
    if (sessionParams.isEmpty() &&
        ...
        ALOGD("%s: Speculative session param configuration with template id: %d", __func__,
                mLastTemplateId);
...
status_t Camera3Device::createDefaultRequest(camera_request_template_t templateId,
    ...
    ATRACE_CALL();
    ALOGD("%s: for template %d", __FUNCTION__, templateId);
...
status_t Camera3Device::waitUntilDrainedLocked(nsecs_t maxExpectedDuration) {
    switch (mStatus) {
        case STATUS_UNINITIALIZED:
        case STATUS_UNCONFIGURED:
            ALOGD("%s: Already idle", __FUNCTION__);
            return OK;
        case STATUS_CONFIGURED:
    ...
    ALOGD("%s: Camera %s: Waiting until idle (%" PRIi64 "ns)", __FUNCTION__, mId.string(),
            maxExpectedDuration);
...
status_t Camera3Device::internalPauseAndWaitLocked(nsecs_t maxExpectedDuration) {
    ...
    ALOGD("%s: Camera %s: Internal wait until idle (% " PRIi64 " ns)", __FUNCTION__, mId.string(),
          maxExpectedDuration);
    status_t res = waitUntilStateThenRelock(/*active*/ false, maxExpectedDuration);
    if (res != OK) {
...
status_t Camera3Device::internalResumeLocked() {
    ...
    ALOGD("%s: Camera %s: Internal wait until active (% " PRIi64 " ns)", __FUNCTION__, mId.string(),
            kActiveTimeout);
...
status_t Camera3Device::triggerAutofocus(uint32_t id) {
    ...
    ALOGD("%s: Triggering autofocus, id %d", __FUNCTION__, id);
...
status_t Camera3Device::triggerCancelAutofocus(uint32_t id) {
    ...
    ALOGD("%s: Triggering cancel autofocus, id %d", __FUNCTION__, id);
    // Mix-in this trigger into the next request and only the next request.
    RequestTrigger trigger[] = {
        {
            ANDROID_CONTROL_AF_TRIGGER,
...
status_t Camera3Device::triggerPrecaptureMetering(uint32_t id) {
    ...
    ALOGD("%s: Triggering precapture metering, id %d", __FUNCTION__, id);
    // Mix-in this trigger into the next request and only the next request.
    RequestTrigger trigger[] = {
        {
...
status_t Camera3Device::flush(int64_t *frameNumber) {
    ATRACE_CALL();
    ALOGD("%s: Camera %s: Flushing all requests", __FUNCTION__, mId.string());
    Mutex::Autolock il(mInterfaceLock);
...
status_t Camera3Device::prepare(int maxCount, int streamId) {
    ATRACE_CALL();
    ALOGD("%s: Camera %s: Preparing stream %d", __FUNCTION__, mId.string(), streamId);
    Mutex::Autolock il(mInterfaceLock);
    Mutex::Autolock l(mLock);
...
status_t Camera3Device::tearDown(int streamId) {
    ATRACE_CALL();
    ALOGD("%s: Camera %s: Tearing down stream %d", __FUNCTION__, mId.string(), streamId);
    Mutex::Autolock il(mInterfaceLock);
    Mutex::Autolock l(mLock);
...
status_t Camera3Device::addBufferListenerForStream(int streamId,
    ...
    ATRACE_CALL();
    ALOGD("%s: Camera %s: Adding buffer listener for stream %d", __FUNCTION__, mId.string(), streamId);
    Mutex::Autolock il(mInterfaceLock);
    Mutex::Autolock l(mLock);
...
void Camera3Device::notifyStatus(bool idle) {
...

    {
        ...
        ALOGD("%s: Camera %s: Now %s, pauseState: %s", __FUNCTION__, mId.string(),
...
status_t Camera3Device::setConsumerSurfaces(int streamId,
    ...
    ATRACE_CALL();
    ALOGD("%s: Camera %s: set consumer surface for stream %d",
...
status_t Camera3Device::configureStreamsLocked(int operatingMode,
    ...
    if (!mNeedConfig) {
        ALOGD("%s: Skipping config, no stream changes", __FUNCTION__);
        return OK;
    }
    ...
    ALOGD("%s: Camera %s: Starting stream configuration", __FUNCTION__, mId.string());
    ...
    ALOGD("%s: Camera %s: Stream configuration complete", __FUNCTION__, mId.string());
...
status_t Camera3Device::addFakeStreamLocked() {
    ...
    ALOGD("%s: Camera %s: Adding a fake stream", __FUNCTION__, mId.string());
...
status_t Camera3Device::tryRemoveFakeStreamLocked() {
    ...
    ALOGD("%s: Camera %s: Removing the fake stream", __FUNCTION__, mId.string());
        ...
        if (err.isOk()) {
            switch (callStatus) {
                ...
                case hardware::camera::common::V1_0::Status::METHOD_NOT_SUPPORTED:
                    ...
                    break;
                default:
                    ALOGD("%s: Reconfiguration query failed: %d", __FUNCTION__, callStatus);
                    ret = true;
            }
        } else {
...
status_t Camera3Device::HalInterface::configureStreams(const camera_metadata_t *sessionParams,
    ...
    if (mHidlSession_3_7 != nullptr) {
        ALOGD("%s: v3.7 device found", __FUNCTION__);
        ...
        ALOGD("%s: v3.6 device found", __FUNCTION__);
        ...
        ALOGD("%s: v3.5 device found", __FUNCTION__);
        device::V3_5::StreamConfiguration requestedConfiguration3_5;
        requestedConfiguration3_5.v3_4 = requestedConfiguration3_4;
        requestedConfiguration3_5.streamConfigCounter = mNextStreamConfigCounter++;
        ...
        ALOGD("%s: v3.4 device found", __FUNCTION__);
        ...
        ALOGD("%s: v3.3 device found", __FUNCTION__);
        ...
        ALOGD("%s: v3.2 device found", __FUNCTION__);
...
status_t Camera3Device::RequestThread::queueRequestList(
    ...
    if (lastFrameNumber != NULL) {
        *lastFrameNumber = mFrameNumber + mRequestQueue.size() - 1;
        ALOGD("%s: requestId %d, mFrameNumber %" PRId32 ", lastFrameNumber %" PRId64 ".",
...
status_t Camera3Device::RequestThread::clear(
    ...
    ATRACE_CALL();
    Mutex::Autolock l(mRequestLock);
    ALOGD("RequestThread::%s:", __FUNCTION__);
...
bool Camera3Device::RequestThread::updateSessionParameters(const CameraMetadata& settings) {
    ...
    for (auto tag : mSessionParamKeys) {
        ...
        if (entry.count > 0) {
            ...
            if (isDifferent) {
                ALOGD("%s: Session parameter tag id %d changed", __FUNCTION__, tag);
            ...
            ALOGD("%s: Session parameter tag id %d removed", __FUNCTION__, tag);
    ...
    for (size_t i = 0; i < mNextRequests.size(); i++) {
        ...
        if (newRequest) {
            ...
            IF_ALOGV() {
                ...
                if (e.count > 0) {
                    ALOGD("%s: Request (frame num %d) had AF trigger 0x%x",
                          __FUNCTION__,
                          halRequest->frame_number,
                          e.data.u8[0]);
                }
        ...
        for (size_t j = 0; j < captureRequest->mOutputStreams.size(); j++) {
            ...
            if (mUseHalBufManager) {
                if (outputStream->isAbandoned()) {
                    ALOGD("%s: stream %d is abandoned, skipping request", __FUNCTION__, streamId);
                    return TIMED_OUT;
                }
                ...
                if (res != OK) {
                    ...
                    // error
                    ALOGD("RequestThread: Can't get output buffer, skipping request:"
    ...
    for (auto& nextRequest : mNextRequests) {
        ...
        // Remove yet-to-be submitted inflight request from inflightMap
        {
          ...
          if (parent != NULL) {
              ...
              if (idx >= 0) {
                  ALOGD("%s: Remove inflight request from queue: frameNumber %" PRId64,
...
sp<Camera3Device::CaptureRequest>
    ...
    while (mRequestQueue.empty()) {
        ...
        if ((mRequestQueue.empty() && mRepeatingRequests.empty()) ||
            ...
            Mutex::Autolock pl(mPauseLock);
            if (mPaused == false) {
                ALOGD("%s: RequestThread: Going idle", __FUNCTION__);
                mPaused = true;
    ...
    Mutex::Autolock pl(mPauseLock);
    if (mPaused) {
        ALOGD("%s: RequestThread: Unpaused", __FUNCTION__);
        sp<StatusTracker> statusTracker = mStatusTracker.promote();
        if (statusTracker != 0) {
...
bool Camera3Device::RequestThread::waitIfPaused() {
    ...
    while (mDoPause) {
        if (mPaused == false) {
            mPaused = true;
            ALOGD("%s: RequestThread: Paused", __FUNCTION__);
            if (mNotifyPipelineDrain) {
                mInterface->signalPipelineDrain(mStreamIdsToBeDrained);
                mNotifyPipelineDrain = false;
                mStreamIdsToBeDrained.clear();
...
void Camera3Device::RequestThread::unpauseForNewRequests() {
    ...
    if (!mDoPause) {
        ALOGD("%s: RequestThread: Going active", __FUNCTION__);
        if (mPaused) {
            sp<StatusTracker> statusTracker = mStatusTracker.promote();
            if (statusTracker != 0) {
                statusTracker->markComponentActive(mStatusId);
...
status_t Camera3Device::PreparerThread::prepare(int maxCount, sp<Camera3StreamInterface>& stream) {
    ...
    if (res == OK) {
        ...
        ALOGD("%s: Stream %d already prepared", __FUNCTION__, stream->getId());
        if (listener != NULL) {
    ...
    if (!mActive) {
        ...
        mCancelNow = false;
        mActive = true;
        ALOGD("%s: Preparer stream started", __FUNCTION__);
    }

    // queue up the work
    mPendingStreams.emplace(maxCount, stream);
    ALOGD("%s: Stream %d queued for preparing", __FUNCTION__, stream->getId());
...
bool Camera3Device::PreparerThread::threadLoop() {
    status_t res;
    {
        ...
        if (mCurrentStream == nullptr) {
            ...
            if (mPendingStreams.empty()) {
                ALOGD("%s: Preparer stream out of work", __FUNCTION__);
            ...
            ALOGD("%s: Preparing stream %d", __FUNCTION__, mCurrentStream->getId());
        } else if (mCancelNow) {
            mCurrentStream->cancelPrepare();
            ATRACE_ASYNC_END("stream prepare", mCurrentStream->getId());
            ALOGD("%s: Cancelling stream %d prepare", __FUNCTION__, mCurrentStream->getId());
            mCurrentStream.clear();
            mCancelNow = false;
            return true;
        }
    ...
    sp<NotificationListener> listener = mListener.promote();
    if (listener != NULL) {
        ALOGD("%s: Stream %d prepare done, signaling listener", __FUNCTION__,
